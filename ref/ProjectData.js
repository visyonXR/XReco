...
import { faTimes, faCubes, faCog, faHouse, faFileCode, faSave, faVideo, faSeedling, faFileImage, faFileAudio, faFileVideo } from '@fortawesome/free-solid-svg-icons';
    ...
    const capsules = [{
      ...
      Scene: {
        ...
        Objects: modelFiles.map((file) => ({
          ...
          Transform: {
            Position: [
              parseFloat(file.position[0]).toFixed(3),
              parseFloat(file.position[1]).toFixed(3),
              parseFloat(file.position[2]).toFixed(3),
            ],
            Rotation: [
              radiansToDegrees(parseFloat(file.rotation[0])).toFixed(3),
              radiansToDegrees(parseFloat(file.rotation[1])).toFixed(3),
              radiansToDegrees(parseFloat(file.rotation[2])).toFixed(3),
            ],
            Scale: [
              parseFloat(file.scale[0]).toFixed(3),
              parseFloat(file.scale[1]).toFixed(3),
              parseFloat(file.scale[2]).toFixed(3),
            ],
          },
        })),
        Triggers: modelFiles
          .filter(file => file.triggers !== null && file.triggers.length > 0)
          .flatMap(file => 
            file.triggers.map(trigger => ({
              Type: `${trigger.actionText} - ${trigger.reactionText}`,
              TriggerParameters: [
                ...(trigger.actionText.startsWith('Time') ? [
                  `action: ${trigger.actionText}`,
                  `timeValue: ${trigger.timeValue}`
                ] : []),
                ...(trigger.actionText.startsWith('Touch') ? [
                  `action: ${trigger.actionText}`,
                  `timeValue: ${trigger.timeValue}`
                ] : []),
                ...(trigger.reactionText.startsWith('Transform') ? [
                  `reaction: ${trigger.reactionText}`,
                  `transform: ${JSON.stringify(trigger.transform)}`
                ] : []),
                ...(trigger.reactionText.startsWith('Visibility') ? [
                  `reaction: ${trigger.reactionText}`,
    ...
    <div className='contenedor-form'>
      <div className="boton-persIcon-container">
        ...
        <ConfirmationModal
          ...
          message="You have unsaved changes. If you leave, all changes will be lost. Are you sure you want to continue?"          
